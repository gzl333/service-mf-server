...
// // groupQuota
// const quotas = computed(() => store.getGroupQuotasByGroupIdByStatus(groupId, 'all'))
// // group quota applications
// const applications = computed(() => store.getGroupQuotaApplicationsByGroupId(groupId))
                  ...
                  <div class="col-10">
                    <div class="row justify-center items-center" style="height: 70px">
                      ...
                      <div v-if="i18n.global.locale==='zh'" class="column justify-center items-center">
                        <div class="col">{{ new Date(group.creation_time).toLocaleString(i18n.global.locale).split(' ')[0] }}</div>
                        <div class="col">{{ new Date(group.creation_time).toLocaleString(i18n.global.locale).split(' ')[1] }}</div>
                      </div>
...
<!--                           :label="tc('配额申请记录')"/>-->
                  ...
                  <q-btn v-show="(tab==='quota'|| tab==='application')&& group.myRole !== 'member'" class="col-shrink"
                         ...
                         unelevated dense padding="xs"
                         color="primary"
                         :to="{path: '/my/group/quota/apply', query: {group: groupId} }">
                    申请新配额
                  </q-btn>
